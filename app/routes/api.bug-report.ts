...
const bugReportSchema = z.object({
  ...
  environmentInfo: z
    .object({
      ...
      uncubedVersion: z.string().optional(),
...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.uncubedVersion) {
      body += `- uncubed.diy: ${data.environmentInfo.uncubedVersion}\n`;
    }
  ...
  body += '---\n*Submitted via uncubed.diy bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/uncubed.diy';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'uncubed.diy-bug-reporter',
    });
