    ...
    const components = useMemo(() => {
      return {
        div: ({ className, children, node, ...props }) => {
          ...
          if (className?.includes('__uncubedArtifact__')) {
          ...
          if (className?.includes('__uncubedSelectedElement__')) {
            const messageId = node?.properties.dataMessageId as string;
            ...
            return (
              <div className="bg-uncubed-elements-background-depth-3 border border-uncubed-elements-borderColor rounded-lg p-3 my-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-mono bg-uncubed-elements-background-depth-2 px-2 py-1 rounded text-uncubed-elements-textTer">
                    {elementData?.tagName}
                  </span>
                  {elementData?.className && (
                    <span className="text-xs text-uncubed-elements-textSecondary">.{elementData.className}</span>
                  )}
                </div>
                <code className="block text-sm !text-uncubed-elements-textSecondary !bg-uncubed-elements-background-depth-2 border border-uncubed-elements-borderColor p-2 rounded">
          ...
          if (className?.includes('__uncubedThought__')) {
            return <ThoughtBox title="Thought process">{children}</ThoughtBox>;
          }

          if (className?.includes('__uncubedQuickAction__') || dataProps?.dataUncubedQuickAction) {
        ...
        button: ({ node, children, ...props }) => {
          ...
          if (
            dataProps?.class?.toString().includes('__uncubedQuickAction__') ||
            dataProps?.dataUncubedQuickAction === 'true'
          ) {
            ...
            return (
              <button
                className="rounded-md justify-center px-3 py-1.5 text-xs bg-uncubed-elements-item-backgroundAccent text-uncubed-elements-item-contentAccent opacity-90 hover:opacity-100 flex items-center gap-2 cursor-pointer"
                data-type={type}
 ...
 * const input = "```xml\n<div class='__uncubedArtifact__'></div>\n```";
 * stripCodeFenceFromArtifact(input);
 * // Returns: "\n<div class='__uncubedArtifact__'></div>\n"
 *
 * @remarks
 * - Only removes code fences that directly wrap an artifact (marked with __uncubedArtifact__ class)
 * - Handles code fences with optional language specifications (e.g. ```xml, ```typescript)
 * - Preserves original content if no artifact is found
 * - Safely handles edge cases like empty input or artifacts at start/end of content
 */
export const stripCodeFenceFromArtifact = (content: string) => {
  if (!content || !content.includes('__uncubedArtifact__')) {
    return content;
  }

  const lines = content.split('\n');
  const artifactLineIndex = lines.findIndex((line) => line.includes('__uncubedArtifact__'));
