  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__uncubedThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__uncubedArtifact__', '__uncubedThought__', '__uncubedQuickAction', '__uncubedSelectedElement__'],

      // ['className', '__uncubedThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__uncubedArtifact__', '__uncubedThought__', '__uncubedQuickAction'],
    ],
  },
  strip: [],
};
