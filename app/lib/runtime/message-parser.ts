...
import type { UncubedArtifactData } from '~/types/artifact';
import { createScopedLogger } from '~/utils/logger';
import { unreachable } from '~/utils/unreachable';

const ARTIFACT_TAG_OPEN = '<uncubedArtifact';
const ARTIFACT_TAG_CLOSE = '</uncubedArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<uncubedAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</uncubedAction>';
const UNCUBED_QUICK_ACTIONS_OPEN = '<uncubed-quick-actions>';
const UNCUBED_QUICK_ACTIONS_CLOSE = '</uncubed-quick-actions>';

const logger = createScopedLogger('MessageParser');

export interface ArtifactCallbackData extends UncubedArtifactData {
  messageId: string;
}
...
interface MessageState {
  ...
  currentArtifact?: UncubedArtifactData;
  currentAction: UncubedActionData;
  actionId: number;
}
...
export class StreamingMessageParser {
  ...
  parse(messageId: string, input: string) {
    ...
    while (i < input.length) {
      if (input.startsWith(UNCUBED_QUICK_ACTIONS_OPEN, i)) {
        console.log('input:', input.slice(i));

        const actionsBlockEnd = input.indexOf(UNCUBED_QUICK_ACTIONS_CLOSE, i);

        if (actionsBlockEnd !== -1) {
          const actionsBlockContent = input.slice(i + UNCUBED_QUICK_ACTIONS_OPEN.length, actionsBlockEnd);

          // Find all <uncubed-quick-action ...>label</uncubed-quick-action> inside
          const quickActionRegex = /<uncubed-quick-action([^>]*)>([\s\S]*?)<\/uncubed-quick-action>/g;
          let match;
          const buttons = [];
          ...
          i = actionsBlockEnd + UNCUBED_QUICK_ACTIONS_CLOSE.length;
          continue;
        }
      }
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as UncubedAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as UncubedAction,
              });
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    'class="__uncubedQuickAction__"',
    'data-uncubed-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__uncubedQuickAction__\" data-uncubed-quick-action=\"true\">${buttons.join('')}</div>`;
}
