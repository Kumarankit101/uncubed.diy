...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<uncubedArtifact') || input.includes('</uncubedArtifact>');
  }
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<uncubedArtifact id="${artifactId}" title="${title}" type="bundled">
<uncubedAction type="file" filePath="${filePath}">
${content}
</uncubedAction>
</uncubedArtifact>`;
  }
