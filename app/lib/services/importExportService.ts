...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          uncubed_user_profile: this._safeGetItem('uncubed_user_profile'),
          uncubed_settings: this._safeGetItem('uncubed_settings'),
          uncubed_profile: this._safeGetItem('uncubed_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('uncubed_viewed_features'),
          developer_mode: this._safeGetItem('uncubed_developer_mode'),
        ...
        ui: {
          // Tab configuration
          uncubed_tab_configuration: this._safeGetItem('uncubed_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('uncubed_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('uncubed_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          uncubed_read_logs: this._safeGetItem('uncubed_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('uncubed_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('uncubed_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.uncubed_tab_configuration) {
        try {
          this._safeSetItem('uncubed_tab_configuration', data.ui.uncubed_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'uncubed_acknowledged_debug_issues',
        'uncubed_acknowledged_connection_issue',
        'error_logs',
        'uncubed_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('uncubed_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
