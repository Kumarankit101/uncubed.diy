...
import { extractCurrentContext, extractPropertiesFromMessage, simplifyUncubedActions } from './utils';
...
export async function createSummary(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyUncubedActions(content);
      content = content.replace(/<div class=\\"__uncubedThought__\\">.*?<\/div>/s, '');
